...
def get_inspire_hand_state():
    return inspire_hand_state(
        pos_act=[0 for _ in range(6)],        # 各自由度的执行器位置实际值
        angle_act=[0 for _ in range(6)],       # 各自由度的角度实际值
        force_act=[0 for _ in range(6)],      # 各手指的实际受力
        current=[0 for _ in range(6)],        # 各自由度的执行器的电流值
        err=[0 for _ in range(6)],        # 各自由度的执行器的故障信息
        status=[0 for _ in range(6)],        # 各自由度的状态信息
        temperature=[0 for _ in range(6)],        # 各自由度的执行器的温度
    ) 

def get_inspire_hand_ctrl():
    return inspire_hand_ctrl(
        pos_set=[0 for _ in range(6)],        # 各自由度的执行器位置设置值
        angle_set=[0 for _ in range(6)],       # 各自由度的角度设置值
        force_set=[0 for _ in range(6)],      # 各自由度的力控阈值设置值
        speed_set=[0 for _ in range(6)],        # 各自由度的速度设置值
        mode=0b0000
    ) 

defaut_ip='192.168.123.211'
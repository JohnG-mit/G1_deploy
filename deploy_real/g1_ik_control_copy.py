...
class Mode(Enum):
    ...
    WAIT_SEQ_B = auto()
    DEBUG     = auto()          # Debug mode for manual joint input
# -----------------------------------------------------------------------------
# G1 Arm controller
# -----------------------------------------------------------------------------
class G1HighlevelArmController:
    def __init__(self, record_dir="records", history_len=3):
        ...
        self.debug_mode_active = False  # Debug模式标志 
    ...
    def hand_state_worker(self, ip, LR, hand_state_dict, control_dt):
        ...
        try:
            ...
            print(f"[STOP]: 'Ctrl+C' detected in hand_state_worker ({LR}), exiting...")
        except Exception as e:
            print(f"[ERROR]: Exception in hand_state_worker ({LR}): {e}")
    ...
    def _send_hand_joint(self, LR: str):
        '''This part in charge of sending all the ctrl value to the lowlevel motor for the hand.'''
        ...
        if LR == 'l':
            ...
            print(f"[ERROR]: Invalid LR value: {LR}. Must be 'l' or 'r'.")
    
    def _open_hand(self, LR: str):
        '''Open the hand'''
        with self.hand_control_lock:
            if LR == 'l':
    ...
    def play_sequence_a(self):
        ...
        self._open_hand('l')
        print("[A-SEQUENCE] Open right hand, wait 3s")
        time.sleep(3.0)
        print("[A-SEQUENCE] Close right hand")
        self._close_hand('l')
        time.sleep(0.5)
